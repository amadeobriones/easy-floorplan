...
export function furnitureForm(f: Furniture): FormSpec {
  ...
  if (f.type === "sectional") {
    fields.push({
      name: "hand",
      label: "Chaise side",
      helper: "Which side the chaise extends on, facing the sofa from the front",
  ...
  return {
    ...
    toPatch(patch) {
      ...
      // leaving it behind in the config when the type changes.
      if ("type" in out && out.type !== "sectional") out.hand = undefined;
      return out;
    },
  };
}
