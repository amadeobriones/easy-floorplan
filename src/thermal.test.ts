...
function fakeFloor(areas: Area[]): Floor {
  return { id: "f1", name: "F1", walls: [], openings: [], items: [], texts: [], furniture: [], trackers: [], areas };
}
...
describe("renderThermalOverlay", () => {
  ...
  it("uses the fixed overlay opacity, not the room's own opacity", () => {
    const v = values(renderThermalOverlay({ ...room, opacity: 0.9 }, DEFAULT_THERMAL_RANGE.min));
...
describe("THERMAL_LAYER", () => {
  ...
  it("render: skips a room with no tempEntity even with hass present", () => {
    const rooms: Area[] = [
      { id: "r1", points: [{ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 10, y: 10 }, { x: 0, y: 10 }] },
    ];
...
describe("thermalLayer registration", () => {
  const cfg = {
    ...
    height: 10,
    floors: [
      fakeFloor([{ id: "r1", points: [{ x: 0, y: 0 }, { x: 1, y: 1 }], tempEntity: "sensor.living_temp" }]),
    ],
  } as unknown as FloorplanCardConfig;
