...
describe("furnitureForm", () => {
  const fur = (over: Partial<Furniture> = {}): Furniture =>
    ({ id: "f", type: "sofa", x: 0, y: 0, w: 100, h: 60, ...over }) as Furniture;
  const names = (f: Furniture) => furnitureForm(f).fields.map((x) => x.name);

  it("offers every furniture type, each with a label", () => {
    for (const t of FURNITURE_TYPES) expect(FURNITURE_LABELS[t], t).toBeTruthy();
  ...
  it("asks for the chaise side only on a sectional", () => {
    expect(names(fur({ type: "sectional" }))).toContain("hand");
    expect(names(fur({ type: "sofa" }))).not.toContain("hand");
  });

  it("defaults the chaise to the right", () => {
    expect(furnitureForm(fur({ type: "sectional" })).data.hand).toBe("right");
    expect(furnitureForm(fur({ type: "sectional", hand: "left" })).data.hand).toBe("left");
  });

  it("drops a stale hand when the sectional becomes something else", () => {
    const { toPatch } = furnitureForm(fur({ type: "sectional", hand: "left" }));
    expect(toPatch({ type: "sofa" })).toMatchObject({ type: "sofa", hand: undefined });
    expect(toPatch({ type: "sectional" })).not.toHaveProperty("hand");
  });
