...
Create `src/areas.ts`. Note: **no imports** — Task 1's functions need none, and the repo's `noUnusedLocals`/`noUnusedParameters` make an unused import a hard `tsc` error. Task 2 adds the `./types`/`./render` imports when `devicesToAdd` uses them.
...
First add the imports at the **top** of `src/areas.ts` (Task 1 left the file import-free):
